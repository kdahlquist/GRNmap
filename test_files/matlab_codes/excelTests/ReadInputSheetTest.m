classdef ReadInputSheetTest < matlab.unittest.TestCase
    ...
    properties (ClassSetupParameter)
        filename = { '4-genes_6-edges_artificial-data_Sigmoidal_estimation_fixb-0_fixP-0_graph',...
                    ...
                    % '4-genes_6-edges_artificial-data_MM_estimation_fixP-0_graph',...
    ...
    methods (Test)
        ...
        function testSimulationTimepointsMatch(testCase)
           for strain_index = 1:length(testCase.GRNstruct.expressionData)
                testCase.assertEqual(length(testCase.GRNstruct.GRNParams.expression_timepoints), length(testCase.GRNstruct.expressionData(strain_index).t),testCase.GRNstruct.inputFile);
                for timepoint = 1: length(testCase.GRNstruct.GRNParams.expression_timepoints)
                   testCase.assertEqual(testCase.GRNstruct.GRNParams.expression_timepoints(timepoint), testCase.GRNstruct.expressionData(strain_index).t(timepoint).t,testCase.GRNstruct.inputFile);
                end
           end
        end
