...
if strain_length == 1
    Targets = {[GRNstruct.microData(1).strain ' data'],[GRNstruct.microData(1).strain ' model']};
else
    Targets = cell(1,(strain_length * 2));
    for i = 0:(strain_length-1)
        Targets{2*(i)+1} = [GRNstruct.microData(i+1).strain ' data'];
        Targets{(i+1)*2} = [GRNstruct.microData(i+1).strain ' model'];
    end
end
