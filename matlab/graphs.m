...
if strain_length == 1
    Targets = {[cell2mat(GRNstruct.rawExpressionData(1).strain) ' data'],[cell2mat(GRNstruct.rawExpressionData(1).strain) ' model']};
else
    Targets = cell(1,(strain_length * 2));
    for i = 0:(strain_length-1)
        Targets{2*(i)+1} = [cell2mat(GRNstruct.rawExpressionData(i+1).strain) ' data'];
        Targets{(i+1)*2} = [cell2mat(GRNstruct.rawExpressionData(i+1).strain) ' model'];
    end
end

for qq = 1: strain_length;
    td  = GRNstruct.rawExpressionData(qq).data(1,:);
...
%         error_dn = (log2FC(qq).avg - 1.96*log2FC(qq).stdev);
        for ii=1:GRNstruct.GRNParams.num_genes
            ...
            plot(td,GRNstruct.rawExpressionData(qq).data(ii+1,:),'o','Color',plot_colors(qq,:),'LineWidth',3),axis([tmin tmax -3 3]);
